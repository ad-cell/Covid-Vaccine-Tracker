...
function initializeDate() {
  ...
  for (let i = 1; i <= 6; i++) {
    ...
    let tempYear = year;
    if (month === 1) {
      if ((year % 4 === 0 && year % 100 !== 0)||(year % 4 === 0 && year % 100 === 0 && year%400===0) && tempDate >= 30) {
        tempDate -= 29;
        tempMonth++;
      } else if (tempDate >= 29) {
        tempDate -= 28;
...
document.querySelector(".selectbystate").style.display="none";
    ...
    fetch(checkByPinURL).then(function (response) {
      ...
      response.json().then(function (data) {
        ...
        for (let i = 0; i < data.centers.length; i++) {
          ...
          // adding a row
          let addRow = "<tr>";
          addRow += "<td style='background:#F5F7B2;'><h5>" + data.centers[i].name + "</h5></td>";
          ...
          for (let j = 0; j < 7; j++) {
            ...
            if (idx == tempData.length) {
              ...
              if (tempData[idx][0].substring(1, tempData[idx][0].length - 1) == dates[j]) {
                if (tempData[idx][1] > 0) {
                  addRow += "<td style='background:#C0EDA6;'>Available Capacity : " + tempData[idx][1] + "<br>Min Age : " + tempData[idx][2] + "<br>Vaccine : " + tempData[idx][3] + "</td>";
                }
                else {
